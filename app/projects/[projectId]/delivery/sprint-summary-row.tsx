          ...
          <div className="bg-white rounded-2xl shadow-xl max-w-6xl w-full max-h-[88vh] overflow-y-auto p-7">
            ...
            {editable && (
              <div className="mt-5 rounded-xl border border-slate-200 bg-slate-50/60 p-4">
                <h4 className="text-xs font-semibold text-slate-600 uppercase tracking-wide mb-3">Sprint settings</h4>
            ...
            {closed && isAdmin && (
              <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50/60 p-4">
                <h4 className="text-xs font-semibold text-amber-800 uppercase tracking-wide mb-2">Admin — sprint is closed</h4>
                <p className="text-xs text-amber-700 mb-3">
                  Its PV/EV/AV are frozen and locked from editing. Reopen it to make it editable again — the numbers will be
                  recalculated live until it&rsquo;s closed again.
                </p>
                <button
                  onClick={() =>
                    startReopenTransition(async () => {
                      setReopenError(null);
                      if (!window.confirm(`Reopen "${sprint.name}"? Its frozen PV/EV/AV will go back to being computed live until it's closed again.`)) return;
                      try {
                        await reopenSprint(sprint.id, projectId);
                      } catch (err) {
                        setReopenError(err instanceof Error ? err.message : "Failed to reopen sprint.");
                      }
                    })
                  }
                  disabled={reopenPending}
                  className="rounded-md bg-amber-600 text-white text-sm font-medium px-4 py-2 hover:bg-amber-700 disabled:opacity-50"
                >
                  {reopenPending ? "Reopening..." : "Reopen Sprint"}
                </button>
                {reopenError && <p className="mt-1.5 text-xs text-red-600">{reopenError}</p>}
              </div>
            )}
          </div>
        </div>
